...
export default function MemberCard({ member, index }) {
    // Create URL-friendly slug from member name
    const memberSlug = member.name.toLowerCase().replace(/\s+/g, '-');

    return (
        <Link href={`/team/${memberSlug}`} className="block h-full">
            <motion.div
                ...
                className="h-full"
            >
                <div className="h-full bg-arcade-surface border-4 border-neon-red shadow-[8px_8px_0_#050510] hover:shadow-[12px_12px_0_#ff2a2a] hover:-translate-y-2 transition-all duration-200 p-4 flex flex-col relative group box-glow-red cursor-pointer">
